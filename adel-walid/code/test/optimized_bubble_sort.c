...
void test_optimized_bubble_sort_sorted_array() {
  int arr[] = {1, 2, 3, 4, 5};
...
int main() {
  ...
  // Add a suite for Optimized Bubble Sort
  CU_pSuite suite = CU_add_suite("Optimized Bubble Sort Tests", NULL, NULL);
  if (NULL == suite) {
    CU_cleanup_registry();
    return CU_get_error();
  }
  ...
  if ((NULL == CU_add_test(suite, "Test Optimized Bubble Sort",
                           test_optimized_bubble_sort)) ||
      (NULL == CU_add_test(suite, "Test Sorted Array",
                           test_optimized_bubble_sort_sorted_array))) {
    CU_cleanup_registry();
    return CU_get_error();
  }
