...
int main() {
  ...
  // Add a suite for Quick Sort
  CU_pSuite suite = CU_add_suite("Quick Sort Tests", NULL, NULL);
  if (NULL == suite) {
    CU_cleanup_registry();
    return CU_get_error();
  }
  ...
  if ((NULL == CU_add_test(suite, "Test Quick Sort", test_quick_sort)) ||
      (NULL ==
       CU_add_test(suite, "Test Quick Array", test_quick_sort_sorted_array))) {
    CU_cleanup_registry();
    return CU_get_error();
  }
